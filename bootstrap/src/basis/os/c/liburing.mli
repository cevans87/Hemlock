...
type nbytes       = U32.t
type offset       = U64.t
type dfd          = Fcntl.At.dfd
type path         = string
type mode         = Stat.Mode.t
type poll_mask    = Poll.Event.t
type user_data    = U64.t
type sockfd       = I32.t
type len          = U32.t
type count        = U32.t
type tv_sec       = I64.t
...
type fd_out       = I32.t
type off_out      = I64.t
type splice_flags = Fcntl.Splice.F.t
type olddirfd     = Fcntl.At.dfd
type oldpath      = string
type newdirfd     = Fcntl.At.dfd
type newpath      = string
type nr_args      = U32.t
...
module Io_uring : sig
  ...
  module Sqe : sig
    ...
    val prep_openat : mode -> Fcntl.O.t -> path -> dfd -> t -> unit (* io_uring_prep_openat *)
    ...
    val prep_cancel64  : Ioring.Async_cancel.t -> user_data -> t -> unit (* io_uring_prep_cancel64 *)
    val prep_cancel_fd : Ioring.Async_cancel.t -> fd -> t -> unit (* io_uring_prep_cancel_fd *)
    val prep_connect : Sockaddr.sockaddr -> fd -> t -> unit (* io_uring_prep_connect *)
    val prep_accept  : Socket.Sock.t -> Sockaddr.sockaddr -> fd -> t -> unit (* io_uring_prep_accept *)
    val prep_recv     : Socket.Msg.t -> len -> buf -> sockfd -> t -> unit (* io_uring_prep_recv *)
    val prep_send     : Socket.Msg.t -> len -> buf -> sockfd -> t -> unit (* io_uring_prep_send *)
    val prep_timeout      : Ioring.Timeout.t -> count -> tv_nsec -> tv_sec -> t -> unit (* io_uring_prep_timeout *)
    val prep_link_timeout : Ioring.Timeout.t -> tv_nsec -> tv_sec -> t -> unit (* io_uring_prep_link_timeout *)
    ...
    val prep_renameat : (* io_uring_prep_renameat *)
      Fcntl.At.t -> newpath -> newdirfd -> oldpath -> olddirfd -> t -> unit
    val prep_unlinkat : Fcntl.At.t -> path -> dfd -> t -> unit (* io_uring_prep_unlinkat *)
  ...
  module Params : sig
    ...
    val set_sq_thread_cpu  : U32.t -> t -> unit (* io_uring_params.sq_thread_cpu *)
    val set_sq_thread_idle : U32.t -> t -> unit (* io_uring_params.sq_thread_idle *)
    val set_wq_fd          : fd -> t -> unit (* io_uring_params.wq_fd *)

    val get_sq_entries : t -> U32.t (* io_uring_params.sq_entries *)
    val get_cq_entries : t -> U32.t (* io_uring_params.cq_entries *)
    val get_flags      : t -> Ioring.Setup.t (* io_uring_params.flags *)
    val get_features   : t -> Ioring.Feat.t (* io_uring_params.features *)

    val get_sq_off_head         : t -> U32.t (* io_uring_params.sq_off.head *)
    val get_sq_off_tail         : t -> U32.t (* io_uring_params.sq_off.tail *)
    val get_sq_off_ring_mask    : t -> U32.t (* io_uring_params.sq_off.ring_mask *)
    val get_sq_off_ring_entries : t -> U32.t (* io_uring_params.sq_off.ring_entries *)
    val get_sq_off_flags        : t -> U32.t (* io_uring_params.sq_off.flags *)
    val get_sq_off_dropped      : t -> U32.t (* io_uring_params.sq_off.dropped *)
    val get_sq_off_array        : t -> U32.t (* io_uring_params.sq_off.array *)
    val get_sq_off_user_addr    : t -> U64.t (* io_uring_params.sq_off.user_addr *)

    val get_cq_off_head         : t -> U32.t (* io_uring_params.cq_off.head *)
    val get_cq_off_tail         : t -> U32.t (* io_uring_params.cq_off.tail *)
    val get_cq_off_ring_mask    : t -> U32.t (* io_uring_params.cq_off.ring_mask *)
    val get_cq_off_ring_entries : t -> U32.t (* io_uring_params.cq_off.ring_entries *)
    val get_cq_off_overflow     : t -> U32.t (* io_uring_params.cq_off.overflow *)
    val get_cq_off_cqes         : t -> U32.t (* io_uring_params.cq_off.cqes *)
    val get_cq_off_flags        : t -> U32.t (* io_uring_params.cq_off.flags *)
    val get_cq_off_user_addr    : t -> U64.t (* io_uring_params.cq_off.user_addr *)
  end
  ...
  val setup    : U32.t -> Params.t -> (fd, Errno.t) result (* io_uring_setup *)
  val enter    : fd -> U32.t -> U32.t -> Ioring.Enter.t -> (res, Errno.t) result (* io_uring_enter *)
  ...
  val queue_init      : entries -> Ioring.Setup.t -> (t, Errno.t) result (* io_uring_queue_init *)
