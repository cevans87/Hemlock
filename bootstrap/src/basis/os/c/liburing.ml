...
module Io_uring = struct
  ...
  module Sqe = struct
    ...
    external prep_openat: mode -> Fcntl.O.t -> path -> dfd -> t -> unit
    ...
    external prep_cancel64: Ioring.Async_cancel.t -> user_data -> t -> unit
      = "hemlock__c__liburing__io_uring__sqe__prep_cancel64"

    external prep_cancel_fd: Ioring.Async_cancel.t -> fd -> t -> unit
    ...
    external prep_accept: Socket.Sock.t -> Sockaddr.sockaddr -> fd -> t -> unit
      = "hemlock__c__liburing__io_uring__sqe__prep_accept"

    external prep_recv: Socket.Msg.t -> len -> buf -> sockfd -> t -> unit
      = "hemlock__c__liburing__io_uring__sqe__prep_recv"

    external prep_send: Socket.Msg.t -> len -> buf -> sockfd -> t -> unit
      = "hemlock__c__liburing__io_uring__sqe__prep_send"

    external prep_timeout: Ioring.Timeout.t -> count -> tv_nsec -> tv_sec -> t -> unit
      = "hemlock__c__liburing__io_uring__sqe__prep_timeout"

    external prep_link_timeout: Ioring.Timeout.t -> tv_nsec -> tv_sec -> t -> unit
    ...
    external prep_unlinkat: Fcntl.At.t -> path -> dfd -> t -> unit
  ...
  module Params = struct
    ...
    external set_sq_thread_cpu  : U32.t -> t -> unit
      = "hemlock__c__liburing__io_uring__params__set_sq_thread_cpu"
    external set_sq_thread_idle : U32.t -> t -> unit
    ...
    external get_sq_entries : t -> U32.t
      = "hemlock__c__liburing__io_uring__params__sq_entries"
    external get_cq_entries : t -> U32.t
  ...
  external setup    : U32.t -> Params.t -> (fd, errno) result
    = "hemlock__c__liburing__io_uring__setup"
  external enter    : fd -> U32.t -> U32.t -> Ioring.Enter.t -> (res, errno) result
  ...
  external queue_init: entries -> Ioring.Setup.t -> (t, errno) result
