...
module Open: sig
  ...
  val submit: ?flag:Flag.t -> ?mode:uns -> Path.t -> (t, Errno_deprecated.t) result
  (** [submit ~flag ~mode path] submits an open operation for a file at [path] with [flag] (default
      ...
      operation does not block. Returns a [t] to the open submission or an [Errno_deprecated.t] if the open
  ...
  val complete: t -> (file, Errno_deprecated.t) result
  (** [complete t] blocks until given [t] is complete. Returns a [file] or an [Errno_deprecated.t] if the file
      could not be opened. *)
...
val of_path: ?flag:Flag.t -> ?mode:uns -> Path.t -> (t, Errno_deprecated.t) result
(** [of_path ~flag ~mode path] opens or creates the file at [path] with [flag] (default Flag.R_O)
    ...
    resulting [t] or an [Errno_deprecated.t] if the file could not be opened. *)
...
module Close: sig
  ...
  val submit: file -> (t, Errno_deprecated.t) result
  (** [submit file] submits a close for given [file]. This operation does not block. Returns a
      [t] to the close submission or an [Errno_deprecated.t] if the close could not be submitted. *)

  val submit_hlt: file -> t
  (** [submit file] submits a close for given [file]. This operation does not block. Returns a
      [t] to the close submission or halts if the close could not be submitted. *)

  val complete: t -> Errno_deprecated.t option
  (** [complete t] blocks until the given [t] is complete. Returns [None] or an [Errno_deprecated.t] if file
      could not be closed. *)
...
val close: t -> Errno_deprecated.t option
(** [close t] closes the external mutable Unix file descriptor associated with [t] and returns
    [None] or an [Errno_deprecated.t] if it could not be closed. *)
...
module Read: sig
  ...
  val submit: ?n:uns -> ?buffer:Bytes.Slice.t -> file -> (t, Errno_deprecated.t) result
  (** [submit ?n ?buffer file] submits a read for given [file]. If given, [n] is the maximum read
      ...
      or an [Errno_deprecated.t] if the read could not be submitted. *)
  ...
  val complete: t -> (Bytes.Slice.t, Errno_deprecated.t) result
...
val read: ?n:uns -> ?buffer:Bytes.Slice.t -> t -> (Bytes.Slice.t, Errno_deprecated.t) result
(** [read ?n ?buffer t] reads from given [t]. If given, [n] is the maximum read size and 1024
    ...
    [n]. Returns the [Bytes.Slice.t] into which bytes were read or an [Errno_deprecated.t] if bytes could not
    be read. *)
...
module Write: sig
  ...
  val submit: Bytes.Slice.t -> file -> (t, Errno_deprecated.t) result
  (** [submit bytes file] submits a write for of given [bytes] to given [file]. This operation does
      not block. Returns a [t] to the write submission or an [Errno_deprecated.t] if the write could not be
      submitted. *)
  ...
  (** [complete t] blocks until the given [t] is complete. Returns a [Bytes.Slice.t] of remaining
      bytes that were not written (typically empty) or an [Errno_deprecated.t] if bytes could not be written.
  *)
...
val write: Bytes.Slice.t -> t -> Errno_deprecated.t option
(** [write bytes t] writes [bytes] to [t] and returns [None] or an [Errno_deprecated.t] if bytes could not be
    written. *)
...
val seek: sint -> t -> (uns, Errno_deprecated.t) result
(** [seek i t] seeks the external mutable Unix file descriptor associated with [t] to point to the
    ...
    index relative to the beginning of the file or an [Errno_deprecated.t] if it could not be changed. *)
...
val seek_hd: sint -> t -> (uns, Errno_deprecated.t) result
(** [seek_hd i t] seeks the external mutable Unix file descriptor associated with [t] to point to
    ...
    to the beginning of the file or an [Errno_deprecated.t] if it could not be changed. *)
...
val seek_tl: sint -> t -> (uns, Errno_deprecated.t) result
...
module Stream : sig
  ...
  val write: file -> t -> Errno_deprecated.t option
